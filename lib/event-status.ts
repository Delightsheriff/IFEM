...
export function splitEvents(events: Event[], now: Date) {
  ...
  const past = validEvents
    .filter((event) => new Date(event.endsAt).getTime() < currentTime)
    .toSorted((a, b) => new Date(b.endsAt).getTime() - new Date(a.endsAt).getTime());
...
export function isCompleteSpotlight(event: Event): boolean {
  ...
  if (!spotlight?.heading?.trim() || !spotlight.summary?.trim() || spotlight.media.length === 0) {
    return false;
  }

  return spotlight.media.every((item) => {
    if (item.type === "image") return Boolean(item.url?.trim() && item.alt?.trim());
    return Boolean(item.title?.trim() && item.url?.startsWith("https://") && item.poster?.url?.trim() && item.poster.alt?.trim());
  });
}
