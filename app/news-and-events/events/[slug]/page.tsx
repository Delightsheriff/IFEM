...
export async function generateMetadata({
  ...
  return {
    title,
    description,
    alternates: { canonical: `/news-and-events/events/${slug}` },
    openGraph: {
      title: `${title} | IFEM Education`,
      description,
      url: `/news-and-events/events/${slug}`,
      type: "website",
      images: [
        {
          url: image,
...
export default async function EventPage({
  ...
  const eventSchema = {
    ...
    eventStatus: `https://schema.org/${isPast ? "EventCompleted" : "EventScheduled"}`,
    eventAttendanceMode: `https://schema.org/${event.attendanceMode === "online" ? "OnlineEventAttendanceMode" : "OfflineEventAttendanceMode"}`,
    location:
  ...
  const hostReady = Boolean(
    event.host?.name?.trim() && event.host.role?.trim(),
  );
  const partners =
    event.partnerUniversities?.filter((partner) => partner.name?.trim()) ?? [];
        ...
        <section className="pb-14 md:pb-20">
          <div className="mx-auto max-w-5xl">
            <div className="grid gap-8 lg:grid-cols-[1.2fr_0.8fr] lg:items-start">
              <div>
                <div className="mb-6 flex items-center gap-4">
                  <span
                    className={`inline-flex rounded-full px-3 py-1 text-[10px] font-semibold uppercase tracking-widest ${isPast ? "bg-[#ede8df] text-[#5a5a5a]" : "bg-[#e8f3ec] text-[#1a5c34]"}`}
                  >
                    {isPast ? "Past event" : "Upcoming event"}
                  </span>
                  <span className="text-[10px] font-bold uppercase tracking-[0.18em] text-[#1a5c34]">
                    {getEventFormatLabel(event.format)}
                  </span>
                </div>
                <h1 className="font-serif text-4xl font-bold leading-[1.05] tracking-[-0.025em] text-charcoal md:text-6xl">
                  {event.title}
                </h1>
                ...
                {event.coverImage?.url ? (
                  <div className="relative mx-auto mt-9 aspect-[3/4] w-full max-w-[680px] overflow-hidden rounded-xl border border-[#e2e2de] bg-[#fafaf7]">
                    <Image
            ...
            whatToBring.length > 0 ? (
              ...
              >
                <p className="mb-2 text-[10px] font-bold uppercase tracking-[0.18em] text-[#1a5c34]">
                  Build your visit
                </p>
                <h2
                  id="at-event-heading"
                    ...
                    <div>
                      ...
                      <ul className="mt-3 space-y-2">
                        {highlights.map((item) => (
                          <li
                            key={item}
                            className="flex gap-2 text-sm leading-relaxed text-gray"
                          >
                            <Check
                              aria-hidden="true"
                              className="mt-0.5 h-4 w-4 shrink-0 text-[#1a5c34]"
