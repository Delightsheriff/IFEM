...
export default function DomeGallery({
  ...
  const [activeStory, setActiveStory] = useState<ItemDef | null>(null);
  ...
  const closeOverlay = useCallback(() => {
    ...
    if (originalImg) {
      const imgElement = originalImg.cloneNode() as HTMLImageElement;
      imgElement.style.cssText =
        "width: 100%; height: 100%; object-fit: cover;";
      animatingOverlay.appendChild(imgElement);
    }
  ...
  }, [enlargeTransitionMs, openedImageBorderRadius, grayscale, onStorySelect]);
  ...
  const openItemFromElement = (el: HTMLElement) => {
    ...
    // Check if this story has a video
    const hasVideo = storyData && storyData.videoUrl;
    ...
    if (storyData && storyData.comment) {
      ...
      // Animate testimonial in after image opens
      setTimeout(() => {
